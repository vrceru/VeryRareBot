...
- `/media importplaylist <url>` — staff-only. Bulk-imports every track in a YouTube playlist into VeryRare Media in one shot: each track is confidence-matched against MusicBrainz for real metadata (title/artist/album/artwork) rather than kept as a raw YouTube filename, then queued as its own pre-approved request so the whole album downloads (not just one track). Posts a live progress panel to the channel — completed/failed counts across the whole playlist — that keeps updating on its own as tracks finish, no need to check `/media myrequests` per track.
